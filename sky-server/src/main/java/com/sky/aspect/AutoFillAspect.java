...
public class AutoFillAspect {
    ...
    public void autFill(JoinPoint joinPoint) throws NoSuchMethodException {
        ...
        Object entity = args[0];//(entity是Object类型,没有getset方法,想给里面的东西赋值得用反射)
        ...
        if (operationType == OperationType.INSERT){//反射三要素 : 调用什么对象 形参是什么  返回什么
            ...
            //为4个公共字段赋值 (.getClass()获取该实体对象的class文件)
            //                (.getDeclaredMethod()从此clss文件中获取一个(Declared无论是公共还是私有都可获取)方法的对象
            //                "setCreateTime"获取的方法对象的名字    LocalDateTime.class此方法的返回值类型的class
            ...
            //通过反射给对象属性赋值
                setCreateTime.invoke(entity,now);//将异常以try/catch形式抛出
